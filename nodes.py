...
class IdentifyAbstractions(Node):
    ...
    def exec(self, prep_res):
        context, file_listing_for_prompt, file_count, project_name, language = (
            prep_res  # Unpack project name and language
        )
...
Identify 5-10 core entrypoints, or common modules.

For each:
1. `name`{name_lang_hint}.
2. `description`: describe the overall purpose. Max 100 words{desc_lang_hint}. DO NOT use metaphor.
3. list relevant `file_indices` (integers) with format `idx # path/comment`.

List file indices, paths present in context:
{file_listing_for_prompt}

Format output as YAML list dictionaries:

```yaml
- name: |
    Query Processing{name_lang_hint}
  description: |
    Explains module purpose.
    Is a central dispatcher routing inbound requests to service.{desc_lang_hint}
  file_indices:
    - 0 # path/to/file1.py
...
class AnalyzeRelationships(Node):
    ...
    def exec(self, prep_res):
        context, abstraction_listing, project_name, language = (
            prep_res  # Unpack project name and language
        )
...
1. high-level `summary` of the project's main purpose and functionality. Audience, new developer (onboarding). Max 3 sentences. {lang_hint}. Format markdown. Highlight important concepts.
2. list (`relationships`) describing relationships between modules. For each relationship, specify:
    - `from_module`: Index of the source module (e.g., `0 # module1`)
    - `to_module`: Index of the target module (e.g., `1 # module2`)
    - `label`: label the relationship **max 5 words**{lang_hint} (e.g., "Manages", "Inherits", "Uses").
    Maximize signal to noise ratio. Emphasise public module interefaces.

Format output as YAML:

```yaml
summary: |
  A concise, plain explanation of the project{lang_hint}.
relationships:
  - from_module: 0 # AbstractionName1
    to_module: 1 # AbstractionName2
    label: "Manages"{lang_hint}
  - from_module: 2 # AbstractionName3
    to_module: 0 # AbstractionName1
    label: "Configuration{lang_hint}
  # ... other relationships
```
...
"""
        ...
        for rel in relationships_data["relationships"]:
            ...
            if not isinstance(rel, dict) or not all(
                k in rel for k in ["from_module", "to_module", "label"]
            ):
                raise ValueError(
                    f"Missing keys (expected from_module, to_module, label) in relationship item: {rel}"
                )
            ...
            # Validate indices
            try:
                from_idx = int(str(rel["from_module"]).split("#")[0].strip())
                to_idx = int(str(rel["to_module"]).split("#")[0].strip())
                if not (
...
class OrderChapters(Node):
    ...
    def exec(self, prep_res):
        ...
        prompt = f"""
Given project modules and relationships```` {project_name} ````:

Modules (Index # Name){list_lang_note}:
{abstraction_listing}

Contextual relationships and project summary:
{context}

```` {project_name} ````
Order the notes on purpose and usage of project modules such that relationships between modules emerge sequentially.
Explain high-level, foundational concepts and entrypoints before proceeding to lower-level concepts and implementation details.

Output the ordered list of module indices, includ module name (as comment) for clarity. Use the format `idx # module-name`.
...
class WriteChapters(BatchNode):
    def prep(self, shared):
        ...
        abstractions = shared[
            "abstractions"
        ]  # List of dicts, name/desc potentially translated
        files_data = shared["files"]
        language = shared.get("language", "english")  # Get language
    ...
    def exec(self, item):
        ...
        prompt = f"""
{language_instruction}We're onboarding new developers. Introduce the developer to project `{project_name}` modules. Topic: "{abstraction_name}". This is Chapter {chapter_num}.
...
{file_context_str if file_context_str else "No specific code snippets provided for this module."}
...
- Start with high-level intention. Identify and explain the problem solved by the module{instruction_lang_note}. Explain the use case. Start minimal, then give elaborate example.

- Deconstruct complex modules. Identify key concepts. Explain each concept.{instruction_lang_note}.

- Demonstrate correct usage{instruction_lang_note}. Provide example inputs, outputs as code snippets (if call does not return a value, then describe expected side-effects {instruction_lang_note}).

- Limit code block line count: max 20 lines. Break down larger code blocks into smaller chunks. Prefer minimal examples. Keep It Simple Stupid. Radical Simplicity. Use comments{code_comment_note} to skip non-important implementation details. Explain the outcome of executing the example immediately after{instruction_lang_note}.

- Describe implementation details if it will improve understanding{instruction_lang_note}. Explain the implementation in step sequence using plain, terse language, or psedocode{instruction_lang_note}. You may use simple sequenceDiagram with a dummy example - keep it minimal with at most 5 participants to ensure clarity. If participant name has space, use: `participant QP as Query Processing`. {mermaid_lang_note}.

- IMPORTANT: When referencing other modules described in another chapter, use proper Markdown links, ie. `[Chapter Title](filename.md)`. Use the Complete Tutorial Structure above to find the correct filename and the chapter title{link_lang_note}. Translate the surrounding text.

- Use mermaid diagrams to illustrate complex concepts (```mermaid``` format). {mermaid_lang_note}.

- Assume competent developer, DO NOT use metaphor, assume baseline knowledge. But improve familiartiy, especially when non-obvious, idiosyncratic patterns detected {instruction_lang_note}.

- End chapter with summary listing key learning points {instruction_lang_note} and provide a transition to the next chapter{instruction_lang_note}. Link to next chapter ie. [Next Chapter Title](next_chapter_filename){link_lang_note}.

- Adopt instructional tone to facilitate swift learning. Maximize signal-to-noise. DO NOT bloviate. {tone_note}.
